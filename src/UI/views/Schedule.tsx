...
import { getGameWinner, getPlayerNickName } from "../../domain/player";
        ...
        {gamesSchedule.map((g) => (
          <div
            ...
            className="grid gap-0 sm:gap-4 grid-cols-schedule justify-center items-center rounded-lg m-2 transform 
                                transition duration-500 hover:scale-110 cursor-pointer border-4 border-orange-500 text-xs sm:text-xl lg:text-2xl lg:p-2 font-semibold text-white bg-slate-800"
          >
            ...
            <span className="pr-2 text-center border-r">
              {format(new Date(g.date), "HH:mm")}
            </span>
            <div className="flex p-1 sm:pl-4 justify-end items-center">
              {isPast(new Date(g.date)) ? (
                <div className="flex justify-center items-center">
                  {getGameWinner(g.id, 2, players, seasons) ? (
                    <>
                      <img
                        src={goldMedal}
